...
>
> * архивирование - это отдельный процесс, что логично с точки зрения требования узкой атомарной ответственности одного компонентам системы.
> * ахивируются именно просроченные уведомления, а не события в исходной БД (я вообще не люблю удалять данные из БД, заменяя в своих проектах это действие понятием "скрыть с глаз").
> * в хранилище введен признак обработки события планировщиком, что позволяет останавливать исходное приложение и запускать его заново без ущерба возникновения двойных уведомлений об одном и том же событии, а также реализует возможность инкрементной выборки событий из хранилища только ранее не поставленных в очередь на уведомление.
...
`Перекладчик` обращается к `Источнику` за каналом данных. Обрабатывает канал поэлементно, принимая решение - направлять его в `Точку назначений`, подтверждая этот факт `Источнику`, или же вернуть обратно `Источнику` для обработки при следующем цикле итераций.

> Стало необходимым разработать структуру [дженерика математического множества](internal/backend/transmitter/set.go). Она помогает отслеживать возникновение цикла обработки. Так, когда необработанный элемент поступает опять на вход обработчику внутри одного итеративного цикла, считается, что все элементы обработаны и необходимо перезапросить данные. Для использования "множества" требуется реализовать только функцию хеширования добавляемого во множество элемента.
...
Таким образом остается (основной момент) реализовать в терминах:

* источника `Source[FROM Item]` - метод `DataChannel(context.Context) (<-chan FROM, error)` получения данных (из базы данных - это список событий в Go-канал, а для RabbitMQ - это делает консьюмер);
...
Для чего именно мной сделан `перекладчик`:

* Для главного посыла ДЗ: "Процессы не должны зависеть от конкретной реализации RMQ-клиента". Он, как мне видится, максимально абстрагирован от конечных сущностей, обрабатываемых процессами переноса данных;
* Личного желания разобраться в работе с RabbitMQ на Golang: взяв событие на принятие решения "отправлять"/"архивировать" или "оставить в очереди" его нужно не просто вернуть, а вернуть **в начало** очереди.
* Личного желания в тренировке с интерфейсами, каналами и дженериками в Golang.
...
Все 30 (тестовых) кандидатов поступили из хранилища через Планировщика в соответствующую очередь.
...
Все 10 из 30 (тестовых) кандидатов на архивирование обработаны Архиватором и помещены в соответствующую очередь.
...
Все 10 из оставшихся 20 из 30 (тестовых) кандидатов на рассылку обработаны Рассыльщиком и помещены в соответствующие очереди отосланных и архивированных (я выбрал такую логику "клонирования" в том числе для демонстрации универсальности `Перекладчика`). Осталось еще 10 уведомлений, которые будут разосланы гораздо позже (что заведомо извесно по подготовленным тестовым данным).
