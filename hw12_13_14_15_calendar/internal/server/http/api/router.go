...
func Handlers(logger interfaces.Logger, app interfaces.Applicationer) regexped.RegexpHandlers {
	return regexped.NewRegexpHandlers(
		...
		logger,
		app,
		*regexped.NewRegexpHandler(`/api/version`, __, middleware.Instance().Listen(commonHandlers.ApiVersionHandler{})),
		...
		*regexped.NewRegexpHandler(`/api/events/create/`, __, middleware.Instance().Listen(apiEventsHandlers.ApiEventsCreateHandler{Logger: logger, App: app})),
