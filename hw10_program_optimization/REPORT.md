...
В исходном файле имелись блоки, которые, исходя из моего опыта, являются узким метом реализации текущего алгоритма работы подсвета.
...
Лучше Slice, если не предполагаем читать порционного по `100_000`

```go
type users []User
```

### 2. Ожидание в рамках полнообъемного чтения
...
Для `string(content)` и обратное `[]byte(line)`
...
вероятно можно подобрать методы без последовательных переводов `[]byte` -> `string` -> `[]byte`.
...
необходимо вынести на уровень выше

и `strings.ToLower(strings.SplitN(user.Email, "@", 2)[1])`
...
заменить, например, на (**но и так я не буду делать**, см. далее)
...
Точно ли `user.Email` содержит `@`, если оканчивается на `.domain`?
...
Ведь если он не содержит, то второй элемент (вот этот `strings.SplitN(user.Email, "@", 2)[1]`) даст ошибку

```text
panic: runtime error: index out of range [1] with length 1
```

#### 7.2. К `json` в принципе

А если "дамп" данных битый, то `json.Unmarshal` даст ошибку `InvalidUnmarshalError`.
...
На протяжении вычислений алгоритм производит разбор регулярного выражеени

```go
... := regexp.Match("\\."+domain, ... )
```

вместо того, чтобы один раз "скомпилировать" регулярное выражение и искать удовлетворение уже скомпилированному:

* `func Compile(expr string) (*Regexp, error)` или
* `func MustCompile(str string) *Regexp` (если п.7 доверяем входному `str`)

#### 8.2. Нет поиска выявленного соответствия

Так, сначала устанавливаем факт отнесения `user.Email` к домену

```go
... := regexp.Match("\\."+domain, []byte(user.Email))
```

а потом ищем этот домен после `@`

```go
... strings.SplitN(user.Email, "@", 2) ...
```

Когда можно использовать функцию поиска "под-соответствия" (`submatch`) с использованием регулярных выражений:
...
Если присмотреться ко всему пакету, то видно, что публичны:
...
Сложно сделать какие-то выводы о возможных источниках данных, на которых планируется применять `GetDomainStatSource`.

* Это исключительно строго типизированные данные формата как в `users.dat.zip`?
* Может ли присутствовать адрес электронной почты в каком-то другом строковом поле (например, в `Username`)?
* А может ли быть подан на вход функции MongoDB-дамп с электронными почтами пользователей?
* А может ли быть подан на вход функции CSV-файл с электронными почтами пользователей?
* А может ли быть подан на вход функции лог SMTP-сервера?
* А может ли быть подан на вход функции плоский файл с текстовыми материалами и контактными реквизитами их авторов (адресами электронной почты)?
...
* отказаться от приватных методов подсчета, реализовав метод разбора файла ("парсинга") на основе соответствия искомого объекта шаблону регулярного выражения (адресу электронной почты).
...
Нагрузка на реализованный мной вариант
...
Реализация не достигла требуемых `300ms`, что предположительно связано с локальным железом. Так, исходя из опроса аудитории, исполнение изначальной реализации не выходил за 1 секунду, когда у меня оно было 1.58 секунды.

### Замечание о вариантах конкурентного доступа к Map-структуре

В рамках конкурентного доступа к объекту `DomainStat` для инкрементирования его значения я успещно опробовал:

* подход с `sync.Mutex` (при этом с выводом в отдельную структуру)
* подход через промежуточный `sync/atomic` (особенность в том, что `DomainStat` по условию менять нельзя, а он содержит `int`, а `sync/atomic` имеет их конкретные архитектурные реализации `int32`/`int64` - `AddInt32`/`AddInt64`)
* подход с заполнением `sync.Map` (описание в Приложении)

### Замечание о числе горутин-обработчиков

Число горутин об задаются через Env
...
### Замечание о большом числе построкового буффера
...
Не знаю, почему с 2_000_000 работает. Так, размер файла `users.dat` 17375349 байт после рапаковки `users.dat.zip`, самого архива `users.dat.zip` 5300876 байт, максимальная длина считываемой строки тестового файла 238, строк в файле 100000, тест бенч-тест запускается "произвольное" число раз.

При этом при периодическом запуске теста это значение можно постепенно снижать и снижать.
...
На основе эталонных данных бенч-теста (этот файл еще пригодится для эксперимента):
...
Большой буффер и много горутин - все ОК.
...
Большой буффер в 100000 (строк видимо по файлу) и одна горутина - все ОК.
...
а вот, например, буффер в 100000 и 2 горутинами
...
При чем в результате будут отсутствовать в ключах статистики некоторые домены, а некоторых будет больше, возможно иногда будут присутствовать произвольные короткие подстроки из файла (вообще не удовлетворяющие регулярному выражению электронной почты, прям середина json-строки, в которой почта никак не удовлетворяет регулярному выражению тоже)

Хорошо. Но теперь магия.

Запустим код `main.go` (ровно копия теста `TestBigDataGetDomainStat` сравнения с эталоном на больших данных) и самостоятельно сверим результат статистики с ожидаемым (как бы вне теста)
...
Смотрите - Вот магическое 64000 для `MaxScanTokenSize` в `bufio`
...
Теперь просто 2 горутины и их нестабильное поведение
...
$ MAX_CAPACITY=128000 WORKERS_COUNT=2 go run ./main.go 
    ...
    Let's check it with ethalon
    FAIL
    ,"username" <---- та Важная строка (см. комментарий выше), как она вообще сбда попала ?!

```

Теперь просто берем большое значение и постепенно его снижаем и получаем везде успех теста, вплоть до того на которых у нас были ошибки
...
* Природа допущения на число горутин - мне абсолютно не понятна.
